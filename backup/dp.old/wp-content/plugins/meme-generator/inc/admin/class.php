...
if (!class_exists('memegenAdmin')) {
     ...
     */
      ...
      class memegenAdmin {
        ...
        public function AddMenuTopPage() {
          $default = array(
            ...
            'menu_title' => '',
            'id' => 'id',
            'icon_url' => $this->SelfPath . '/images/menu_icon.png',
            'position' => null
          );
        ...
        /**
          ...
          public function DisplayPage() {
            ...
            echo '<form method="post" action="" enctype="multipart/form-data">
              ...
              <div style="float:right;margin:32px 0 0 0">
                <input type="submit" style="margin-right: 20px;" value="Save changes" name="Submit" class="button" /><br><br>
              </div>
            <br style="clear:both"><br>
            </div>';
            wp_nonce_field( basename(__FILE__), 'MEMEGEN_Admin_Page_Class_nonce' );

            if ($this->saved_flag)
              echo '<div class="alert memegen-success"><p><strong>'.__('Settings saved.', 'memegen').'</strong></p></div>';
          ...
          echo '</div><div style="clear:both"></div><div class="footer_wrap">
              <div style="float:right;margin:0 0 0 0">
                <input type="submit" style="margin-left: 20px;" name="Submit" class="button" value="'.esc_attr(__('Save Changes', 'memegen')).'" />
                <br><br>
              </div>
              <br style="clear:both"><br>
            </div>';
      ...
      public function insert_images() {
        ...
        foreach( $_POST['attachments'] as $attachment_id => $attachment ) {
          ...
          //$li   .= "<a title='" . __( 'Delete this image', 'memegen' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'>" . __( 'Delete', 'memegen' ) . "</a>";
          $li   .= "<a title='" . __( 'Delete this image', 'memegen' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$post_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath. "/images/delete-16.png' alt='" . __( 'Delete', 'memegen' ) . "' /></a>";
      ...
      */
      public function wp_ajax_delete_image() {
        ...
        if ( $ok ){
          ...
          echo json_encode(array('message' => __( 'Cannot delete file. Something\'s wrong.', 'memegen')));
          die();
        }
      }
      ...
      public function show_field_repeater( $field, $meta ) {
        ...
        $meta = isset($this->_saved[$field['id']])? $this->_saved[$field['id']]: '';
          ...
          if (count($meta) > 0 && is_array($meta) ){
            ...
            echo '" alt="'.__('Remove', 'memegen').'" title="'.__('Remove', 'memegen').'" id="remove-'.$field['id'].'"></div>';
            $c = $c + 1;
        ...
        echo '" alt="'.__('Add', 'memegen').'" title="'.__('Add', 'memegen').'" id="add-'.$field['id'].'"><br/></div>';
    ...
    .at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
      ...
      public function show_field_file( $field, $meta ) {
        ...
        $this->show_field_begin( $field, $meta );
          ...
          if ( ! empty( $meta ) ) {
            $nonce = wp_create_nonce( 'at_ajax_delete' );
            echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files', 'memegen') . '</strong></div>';
            echo '<ol class="at-upload">';
            foreach ( $meta as $att ) {
              // if (wp_attachment_is_image($att)) continue; // what's image uploader for?
              echo "<li>" . wp_get_attachment_link( $att, '' , false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}|{$post->ID}|{$field['id']}|{$att}'>" . __( 'Delete', 'memegen' ) . "</a>)</li>";
            }
            echo '</ol>';
          }
          ...
          echo "<div class='at-file-upload-label'>";
            echo "<strong>" . __( 'Upload new files', 'memegen' ) . "</strong>";
          echo "</div>";
          echo "<div class='new-files'>";
            echo "<div class='file-input'>";
              echo "<input type='file' name='{$field['id']}[]' />";
            echo "</div><!-- End .file-input -->";
            echo "<a class='at-add-file button' href='#'>" . __( 'Add more files', 'memegen' ) . "</a>";
      ...
      public function show_field_typo( $field, $meta ) {
        ...
        $html .= "<input type='button' class='at-color-select button' rel='".$field['id']."[color]' value='" . __( 'Select a color', 'memegen' ) . "'/>";
      ...
      public function show_field_color( $field, $meta ) {
        ...
        $this->show_field_begin( $field, $meta );
          ...
          echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color', 'memegen' ) . "'/>";
      ...
      public function save($repeater = false) {
        ...
        //check nonce
        if ( ! check_admin_referer( basename( __FILE__ ), 'MEMEGEN_Admin_Page_Class_nonce') )
          return;
        ...
        foreach ( $this->_fields as $field ) {
          if(!in_array($field['type'],$skip)){
            ...
            // Validate meta value
            if ( class_exists( 'MEMEGEN_Admin_Page_Class_Validate' ) && method_exists( 'MEMEGEN_Admin_Page_Class_Validate', $field['validate_func'] ) ) {
              $new = call_user_func( array( 'MEMEGEN_Admin_Page_Class_Validate', $field['validate_func'] ), $new );
            }
    ...
    require_once(memegen_path .'inc/admin/options.php');
